...
# -*- coding: utf-8 -*-
"""
inject_competitor_parity_card.py
Computes Share of Voice (SoV) using ALL kept domains as denominator, then displays top-N domains.
"""
...
def build_card(rows, top_n, exclude_platforms, extra_excludes):
    if not rows:
        return f"""{CARD_START}
...
{CARD_END}"""
    ...
    if not rows:
        return f"""{CARD_START}
<div class="card sp-card"><div class="card-header"><h2>Competitor Parity</h2></div>
<div class="card-body">{_empty_state("All kept domains excluded by filters; adjust excludes if needed.")}</div></div>
{CARD_END}"""
    ...
    css = (
        ...
        ".kpis{display:grid;grid-template-columns:repeat(3,minmax(120px,1fr));gap:12px;margin-bottom:8px}"
    ...
    kpis = (
        ...
        f"<div><div class='kpi-title'>Rows</div><div class='kpi-value'>{len(rows)}</div></div>"
        "</div>"
    )
    table = _bars_table(items)
    note = "<p class='muted'>SoV% is computed over <b>all kept domains</b> in the dataset (after excludes). Display shows top results only.</p>"
    return f"""{CARD_START}
<div class="card sp-card">
  <div class="card-header"><h2>Competitor Parity</h2>
    <p class="muted">Share of Voice across all kept domains (fixed denominator) + Top-3 SoV</p>
  </div>
...
def main():
    ...
    new_html = _inject(html, build_card(rows, args.top, bool(args.exclude_large_platforms), excludes))
