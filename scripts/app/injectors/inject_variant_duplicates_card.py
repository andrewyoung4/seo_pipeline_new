...
Adds a clear "no action necessary" empty-state when no clusters are found.
"""
...
def _read_rows(path):
    ...
    if not cols["url"]:
        raise SystemExit("Could not find a URL-like column in triage CSV.")
    return rows, cols
...
def _exemplar_row(base, rows, cols):
    ...
    if not ex and urls:
        ex = urls[0]
    return {"base": base, "canonical_target": canon_target, "example_dupe": ex,
            "status": status, "reason": reason, "count": len(urls)}

def _build_table(exemplars):
...
def build_card(exemplars, totals):
    css = (
        "<style>"
    ...
    kpis = (
        "<div class='kpis' style='display:grid;grid-template-columns:repeat(4,minmax(120px,1fr));gap:12px;margin-bottom:12px'>"
        f"<div><div class='kpi-title'>Duplicate clusters</div><div class='kpi-value'>{totals['clusters']}</div></div>"
        f"<div><div class='kpi-title'>URLs in clusters</div><div class='kpi-value'>{totals['urls_in_clusters']}</div></div>"
        f"<div><div class='kpi-title'>All OK</div><div class='kpi-value'>{totals['ok_clusters']}</div></div>"
        f"<div><div class='kpi-title'>With issues</div><div class='kpi-value'>{totals['issue_clusters']}</div></div>"
        "</div>"
    )
    table = _build_table(exemplars) if exemplars else "<p class='muted'>All clear — no variant/parameter duplicate clusters found. No action necessary.</p>"
    return f"""{CARD_START}
<div class="card sp-card">
  <div class="card-header"><h2>Variant URL Duplicates</h2></div>
  <div class="card-body">
    {kpis}
...
def main():
    ...
    new_html = _inject(html_text, build_card(exemplars, totals))
